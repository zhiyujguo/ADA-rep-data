...
ps_npcausal <- readRDS("ps_npcausal.RDS")
...
f <- as.formula("gender ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 + X9 + X10 + X11 + 
                  ...
                  Nov_indic + Dec_indic")
cardnp <- matchit(f, data = matched.card[, !(names(matched.card) %in% c("weights"))],
                  distance = ps_npcausal, method = "nearest", ratio = 1,
                   caliper = 0.1, std.caliper = T)

matched.cardnp = match.data(cardnp)
saveRDS(matched.cardnp, file = "matched.cardnp.RDS")
cardnp_cb <- abs(getStandardizedCovMeanDiffs(matched.cardnp[-c(46,61:77)],matched.cardnp$gender))
