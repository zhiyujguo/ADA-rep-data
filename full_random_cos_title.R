...
full_random_p <- readRDS("full_random_p.RDS")
full_random_cosdist <- matrix(nrow = nrow(full_random_p),ncol=1)
colnames(full_random_cosdist)<-c("cos_dist")
gender_bin <- c(1,0)
for (i in 1:nrow(full_random_p)) {
  # Extract the pair of rows from textmeta_15 using row names
  row_name_1 <- as.character(full_random_p[i, 1])
  row_name_2 <- as.character(full_random_p[i, 2])
  ...
  full_random_cosdist[i] <- pair_distances(dfm_2015, gender_bin, include = "cosine", verbose = FALSE)[, 3]
}

saveRDS(full_random_cosdist, file = "full_random_cosdist.RDS")