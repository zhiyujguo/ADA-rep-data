...
lasso2_match_table <-readRDS("lasso2_match_table.RDS")
lasso2_cosdist <- matrix(nrow = nrow(lasso2_match_table),ncol=1)
colnames(lasso2_cosdist)<-c("cos_dist")
gender_bin <- c(1,0)
for (i in 1:nrow(lasso2_match_table)){
  text_tr <- rbind(textmeta_15[as.numeric(lasso2_match_table[i,1]),],
               textmeta_15[as.numeric(lasso2_match_table[i,2]),])
  ...
  lasso2_cosdist[i] <-pair_distances(dfm_2015,gender_bin, include = "cosine",verbose = FALSE)[,3]
}
saveRDS(lasso2_cosdist, file = "lasso2_cosdist.RDS")