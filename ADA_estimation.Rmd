...
ate_m_ups <- ate(log(matched.cardnp$ups+1), matched.cardnp$gender,
                 matched.cardnp[,names(matched.cardnp) %in% cov_names],
                 nsplits = 2, sl.lib = c("SL.glm","SL.ranger","SL.glmnet","SL.gam"))
ate_m_comm <- ate(log(matched.cardnp$num_comments+1), matched.cardnp$gender,
                  matched.cardnp[,names(matched.cardnp) %in% cov_names],
                 nsplits = 2, sl.lib = c("SL.glm","SL.ranger","SL.glmnet","SL.gam"))

# original scale
ate_m_ups_og <- ate(matched.cardnp$ups, matched.cardnp$gender,
                 matched.cardnp[,names(matched.cardnp) %in% cov_names],
                 nsplits = 2, sl.lib = c("SL.glm","SL.ranger","SL.glmnet","SL.gam"))
ate_m_comm_og <- ate(matched.cardnp$num_comments, matched.cardnp$gender,
                  matched.cardnp[,names(matched.cardnp) %in% cov_names],
                 nsplits = 2, sl.lib = c("SL.glm","SL.ranger","SL.glmnet","SL.gam"))
```
...
se_full_comm <- sqrt(var(log(cov_15$num_comments[cov_15$gender == "1"]+1))/nrow(cov_15[cov_15$gender == "1",])+var(log(cov_15$num_comments[cov_15$gender == "0"]+1))/nrow(cov_15[cov_15$gender == "0",]))


full_ups_tau <- mean(log(cov_15$ups[cov_15$gender == "1"]+1))-
  mean(log(cov_15$ups[cov_15$gender == "0"]+1))
se_full_ups <- sqrt(var(log(cov_15$ups[cov_15$gender == "1"]+1))/nrow(cov_15[cov_15$gender == "1",])+var(log(cov_15$ups[cov_15$gender == "0"]+1))/nrow(cov_15[cov_15$gender == "0",]))

m_comm_tau <- mean(log(matched.cardnp$num_comments[matched.cardnp$gender == "2"]+1))-
  mean(log(matched.cardnp$num_comments[matched.cardnp$gender == "1"]+1))
se_m_comm <- sqrt(var(log(matched.cardnp$num_comments[matched.cardnp$gender == "2"]+1))/nrow(matched.cardnp[matched.cardnp$gender == "2",])+var(log(matched.cardnp$num_comments[matched.cardnp$gender == "1"]+1))/nrow(matched.cardnp[matched.cardnp$gender == "1",]))

m_ups_tau <- mean(log(matched.cardnp$ups[matched.cardnp$gender == "2"]+1))-
  mean(log(matched.cardnp$ups[matched.cardnp$gender == "1"]+1))
se_m_ups <- sqrt(var(log(matched.cardnp$ups[matched.cardnp$gender == "2"]+1))/nrow(matched.cardnp[matched.cardnp$gender == "2",])+var(log(matched.cardnp$ups[matched.cardnp$gender == "1"]+1))/nrow(matched.cardnp[matched.cardnp$gender == "1",]))
```
...
lm_lasso_ups <- lm(log(ups+1)~.-num_comments, data = data_full)
coefci(lm_lasso_ups, vcov = vcovHC(lm_lasso_ups, type = "HC2"))

lm_lasso_comm <- lm(log(num_comments+1)~.-ups, data = data_full)
coefci(lm_lasso_comm, vcov = vcovHC(lm_lasso_comm, type = "HC2"))

# matched
matched.cardnp = matched.cardnp %>%  mutate_at(c("gender","Jan_indic","Feb_indic",
                                                       "Mar_indic","Apr_indic",
                                "May_indic","Jun_indic","Jul_indic","Aug_indic",
                                "Sep_indic","Oct_indic","Nov_indic","Dec_indic"), as.numeric)
x2 <- matrix(nrow = nrow(matched.cardnp), ncol = 71)
for (i in 1:71){
  x2[,i] = scale(matched.cardnp[,-c(46,73:77)][,i],scale = F)
}
x3 <- matched.cardnp$gender*x2
colnames(x3) <- paste(colnames(matched.cardnp[,-c(46,73:77)]), "gender", sep=":")
data_full = cbind(matched.cardnp[, !names(matched.cardnp) %in% c("distance","weights","subclass")] ,x3)
lm_lasso_ups <- lm(log(ups+1)~.-num_comments, data = data_full)
coefci(lm_lasso_ups, vcov = vcovHC(lm_lasso_ups, type = "HC2"))
...
# matched
findex_m = which(matched.cardnp$gender == "1")
